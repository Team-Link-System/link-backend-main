...
type Like struct {
	...
	Post      *Post    `gorm:"foreignKey:PostID;constraint:OnDelete:CASCADE;OnUpdate:CASCADE"`    // 게시물이 삭제되면 연관된 Like도 삭제
	CommentID *uint    `gorm:""`                                                                  // 댓글과의 관계 (null 허용)
	Comment   *Comment `gorm:"foreignKey:CommentID;constraint:OnDelete:CASCADE;OnUpdate:CASCADE"` // 댓글이 삭제되면 연관된 Like도 삭제
